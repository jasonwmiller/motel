...
cargo run -- diff <id_a> <id_b>      # Compare two traces side-by-side
...
- **`src/client/`** — CLI query commands. Each submodule (trace, log, metrics, sql, latency, diff, clear) builds gRPC requests and formats output (Text/JSONL/CSV). `view.rs` supports connecting to multiple servers simultaneously (`--addr` can be specified multiple times), queries all existing data on connect, then subscribes to Follow streams for new data, piping both into a local Store to drive the TUI. Each item is tagged with a `motel.source` resource attribute identifying its origin server. `diff.rs` fetches two traces and renders a comparison table. `import.rs` reads files (JSONL or OTLP protobuf) and sends data to a server via standard OTLP gRPC clients. `mod.rs` contains shared utilities.
- **`src/diff.rs`** — Shared trace diff computation logic (`compute_diff()`, `DiffResult`, `SpanDiff`). Matches spans by `(service_name, span_name)` with positional disambiguation for duplicates.
...
- **`src/tui/`** — ratatui-based interactive UI with three tabs (1:Logs, 2:Traces, 3:Metrics):
  ...
  - `diff_ui.rs` — TUI rendering for side-by-side trace diff view with color-coded duration changes (red=slower, green=faster, blue=added, gray=removed).
  - `event.rs` — Key handling: `f` toggles follow mode, `g` toggles metric graph, `Enter` opens trace timeline, `m` marks a trace for diff, `d` diffs marked trace with selected, `Esc` goes back from timeline/diff, PgUp/PgDn scrolls detail pane, tab numbers 1/2/3.
...
- **`src/cli.rs`** — clap derive command definitions (Server, View, Traces, Logs, Metrics, Sql, ServiceMap, Export, Latency, Diff, Clear, Status, Shutdown, Replay, Import, SkillInstall, Init, Mcp, Config). Output formats: `Text`, `Table`, `Jsonl`, `Csv`. Import formats: `Jsonl`, `OtlpProto`. Signal types: `Traces`, `Logs`, `Metrics`. Each args struct has a `resolve()` method that merges with config.
